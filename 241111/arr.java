// 자바, 파이썬, C와같은 프로그래밍 언어에서는 반복문을 사용할 수 있는 배열을 선언할 때, 선언단계부터 몇개의 데이터를 처리할지 사전 정의
...
// 해결방법 : 참조타입
// heap : 메모리공간 
// 배열 안에있는 첫번째 인덱스값만 알고 있어도 두번째, 세번째 인덱스값은 알아서 해당 주소를 통해서 찾아오는 특징!!!

// 장점 : 읽어오는데 굉장히 효율적인 자료구조
// 장점 : 이미 자료구조의 형식, 사이즈를 가지고 태어나기 때문에 해당 배열의 첫번째 혹은 마지막번째에 순차적으로 값을 추가하는데 효율적

// 배열 단점 : 최초에 배열 데이터를 생성하는 시점에서 배열안에 입력되어야하는 데이터의 총 개수가 미정인경우
...
// 연결리스트의 경우 : 값을 찾아서 불러오는데 배열 대비 생대적인  시간이 걸림

// 어떤 프로젝트 & 제품을 만드는데 있어서 데이터의 입출력 및 편집이 빈번하게 일어나야하는 상황 => 연결리스트 훨씬 효율적인 자료구조
